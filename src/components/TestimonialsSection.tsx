      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t) => (
            <div
              key={t.name}
              className="flex h-full flex-col rounded-2xl bg-white/5 border border-white/10 p-8 transition-all hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5"
            >
              <StarRating rating={t.rating} />
              <p className="mt-6 flex-1 text-muted">&ldquo;{t.review}&rdquo;</p>
              <div className="mt-6 flex items-end gap-4 border-t border-white/10 pt-6">
                <div
                  className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-primary/20 text-sm font-semibold uppercase tracking-wide text-primary ring-2 ring-white/10"
                  aria-hidden
                >
